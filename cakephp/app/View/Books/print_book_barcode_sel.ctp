...
<?php else: ?>
<?php echo $this->html->link('回上一頁',array('controller'=>'books','action'=>'print_book_barcode_sel'));?>&nbsp;
	...
	<table style="padding:0px; margin:0px">
		...
		<tr style="background-color:#FFF;padding:0px; margin:0px">
			...
			<?php while (($col <= 3) && ($record < sizeof($books))): ?>
				<td style="text-align:center;padding:0px; margin:0px;border-bottom:none"><? echo $this->html->image(array('controller' => 'graph', 'action'=> 'book_barcode',$books[$record]['book_instances']['id'])); ?></td>
				<?php $record++; ?>
				<?php $col++; ?>
				<?php $total_div++; ?>
			<?php endwhile; ?>	
